...
const SocialButtons = ({ mode = "signin" }: { mode?: Mode }) => {
  const { t } = useTranslation();
  const verb =
    mode === "signin" ? t("auth.common.signInWith") : t("auth.common.signUpWith");

  const handleDemo = () => toast("Demo mode — OAuth not wired up.", { icon: "🔒" });
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <button
        type="button"
        onClick={handleDemo}
        aria-label={`${verb} ${t("auth.common.google")}`}
        className="flex items-center justify-center gap-2 rounded-lg border border-border dark:border-dark_border px-4 py-2.5 text-body font-medium text-midnight_text dark:text-white bg-white dark:bg-darkmode hover:border-primary hover:bg-heroBg dark:hover:bg-darkheader transition-all duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
      >
        <svg width="20" height="20" viewBox="0 0 23 22" fill="none" aria-hidden="true">
      ...
      <button
        type="button"
        onClick={handleDemo}
        aria-label={`${verb} ${t("auth.common.github")}`}
        className="flex items-center justify-center gap-2 rounded-lg border border-border dark:border-dark_border px-4 py-2.5 text-body font-medium text-midnight_text dark:text-white bg-white dark:bg-darkmode hover:border-primary hover:bg-heroBg dark:hover:bg-darkheader transition-all duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
      >
        <GithubIcon width={20} height={20} aria-hidden="true" />
        <span>{t("auth.common.github")}</span>
      </button>
    </div>
  );
};
